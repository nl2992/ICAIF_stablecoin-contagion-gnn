...
| Precision@k under an alert budget (5-seed) | oracle(545) **0.29±0.04** vs XGB 0.19 (stable); @10 0.18±0.16, @25 0.16±0.07 vs XGB 0/0 (seed-sensitive at tight budgets) | `results/eval/precision_at_k_svb_multiseed.json` (← `scripts/run_precision_at_k_multiseed.py`, seeds 0–4) | `gat_precision_at_k_multiseed` mean/std, `xgboost_precision_at_k`; XGBoost matches single-seed `precision_at_k_svb.json` exactly |
