...
def fig_network_snapshot():
    ...
    ax.axis("off")
    fig.tight_layout(); fig.savefig(OUT / "fig6_network_snapshot.png"); plt.close(fig)
    print("fig6 ok")
...
def fig_multiseed_ci():
    ...
    ax.bar(range(len(models)), means, yerr=stds, capsize=5,
           color=[ps.LADDER[m] for m in models], edgecolor="k", linewidth=0.5)
    ...
    ax.set_title("Day-ahead prediction: graph attention wins")
    ax.legend(); ax.grid(axis="y", alpha=0.25)
    fig.tight_layout(); fig.savefig(OUT / "fig7_multiseed_ci.png"); plt.close(fig)
    print("fig7 ok")
...
def fig_roc_heatmap():
    ...
    M = M.loc[order]
    fig, ax = plt.subplots(figsize=ps.SINGLE)
    im = ax.imshow(M.values, cmap=ps.DIV_CMAP, vmin=0.35, vmax=0.7, aspect="auto")
    ...
    ax.set_title("Ranking skill by model and horizon")
    ax.grid(False)
    fig.tight_layout(); fig.savefig(OUT / "fig8_roc_heatmap.png"); plt.close(fig)
    print("fig8 ok")
