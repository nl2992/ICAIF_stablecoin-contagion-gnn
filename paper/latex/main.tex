...
Stablecoins are now systemic: the GENIUS Act (US) and MiCA (EU) both make reserve adequacy and
...
\emph{GNNs for on-chain contagion} (e.g.\ ICAIF'25 Uniswap-v3 bridge-swap link prediction)
build temporal directed graphs of pools/venues and predict propagation, but evaluate
\emph{prediction}, not causal hub importance. \emph{Agent-based models of crypto/LOB markets}
(ICAIF'24 liquidity-spoofing; JaxMARL-HFT) note that ABMs are hard to calibrate and use them for
mechanism and policy. \emph{Intrinsic interpretability / XAI} (ProtoHedge; critiques of post-hoc
explanation) motivate causal, not merely attributive, explanations. We connect these: a GNN
predicts, a contagion model causally validates, and the disagreement is the finding.
...
We run a model ladder---majority, persistence, logistic regression, XGBoost, a GRU, GraphSAGE,
and GAT---on identical samples, with a leakage-safe held-out SVB cluster and leave-one-cluster-out
(LOCO).
...
\paragraph{Ablation.} Holding architecture fixed and removing all edges
(Table~\ref{tab:ablation}): node-only XGBoost $0.271 \to$ GNN-no-edges $0.35$--$0.39$ (the
\emph{architecture}) $\to$ GNN-real-edges. The directed lead-lag \textbf{edges add $+0.100$
PR-AUC for GAT} but only $+0.009$ for GraphSAGE---the contagion signal lives in the edges and is
what \emph{attention} exploits.

\begin{table}[t]
\caption{Graph ablation, held-out SVB @ 24h (5 seeds). The edges---not just the neural
architecture---carry the signal, for GAT.}
\label{tab:ablation}
\begin{tabular}{lrr}
\toprule
...
Fig.~\ref{fig:hubmap})---the input to the causal test.
...
model}: per-asset near-critical mean-reverting peg dynamics with a directed transmission matrix
$W$ and an origin-driven panic channel,
\begin{equation}
...
$100\%$. (2) \textbf{Adaptive agents}: switching on an endogenous adaptive redeemer (redemptions
accelerate past a threshold) amplifies contagion $7.7\times$, yet the causal conclusions are
unchanged---a Lucas-critique check. Notably the \emph{intervention} ranking flips: a circuit
breaker overtakes reserve-strengthening ($95\%$ vs $30\%$) once runs are self-reinforcing, so
static-agent analysis would mis-rank policies. (3) \textbf{Structure derivation}: exposure-based
...
repositories; see each repo's \texttt{RESULTS.md} and the command list therein.
